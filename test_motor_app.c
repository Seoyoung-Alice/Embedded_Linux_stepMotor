...
int main(int argc, char **argv)
{
	...
	motor_turn = atoi(argv[3]);
	if(motor_turn<0 || motor_turn>255){
		printf("Invalid motor turn!\n");
		usage(argv[0]);
		return -1;
	}
#if 0
	motor_turn = atoi(argv[4]);
		if(motor_turn<0 || motor_turn>255){
		printf("Invalid motor turn!\n");
		usage(argv[0]);
		rwrite(dev, motor_state, 3);eturn -1;
	}
#endif
...
//	motor_state[3] = (unsigned char)motor_turn;
...
void usage(char* dev_info){
	printf("<Usage> %s [Motor Action] [Motor Direction] [Speed]\n", dev_info);
	printf("Motor Action : 0 - Stop / 1 - Start\n");
	printf("Motor Direction : 0 - Left / 1 - Right\n");
	printf("Motor turn : 0 ~ 250\n");
	printf("ex) %s 1 0 5\n", dev_info);
}
